...
exports.top100 = async(req,res)=>{
 try{
   const profiles = await Profile.find({}).limit(100).sort({ nbFollowers: 'desc' });
   return res.json(profiles);
 }
 catch (error) {
   console.log(error)
...
exports.numberToScrap = async (req, res) => {
  try {
    const nbProfiles = await Profile.countDocuments({nbFollowers:null});
    return res.json(nbProfiles);
  }
  catch (error) {
    console.log(error)
  }
